...
3. 가장 왼편 벡터(`idx_l+1 - idx_l`)와 가장 오른편 벡터(`idx_r - idx_r+1`)를 사용해 기준 벡터(최초의 기준 벡터는 x축 법선벡터)와의 각을 계산한 뒤, 각이 더 작은 방향으로 탐색을 진행한다.
...
2. 점이 컨벡스 헐 내에 존재하는지 판별한다:
    ...
    1. 임의의 점이 기준점의 왼쪽 벡터와의 외적 결과가 음수라면, 해당 점은 도형 외부에 있는 것이다.
    2. 임의의 점이 기준점의 오른쪽 벡터와의 외적 결과가 양수라면, 해당 점은 도형 외부에 있는 것이다.
    3. 이분탐색을 통해 점이 위치할 수 있는 영역을 찾는다. 만약 컨벡스 헐의 개수가 `n`개라면, 이분탐색의 범위는 `[1, n)`이 된다.
    4. 임의의 점(`p`)이 위치할 수 있는 영역을 발견했다면, `convex_hull[idx], p, convex_hull[idx+1]` 세 점에 대해 외적한다:
